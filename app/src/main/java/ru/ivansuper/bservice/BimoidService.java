...
public class BimoidService extends Service implements OnSharedPreferenceChangeListener, Callback {
    ...
    private Notification getNotification() {
        ...
        if (profiles != null) {
            if (profiles.list.size() > 1) {
                notification.setLatestEventInfo(resources.ctx, text, utilities.match(getString(R.string.s_main_notify), new String[]{String.valueOf(profiles.getConnectedProfilesCount()), String.valueOf(profiles.list.size())}), contentIntent);
            } else {
    ...
    private Notification getUnreadedNotification(MessagesDump dump) {
        CharSequence text = getString(R.string.s_has_unreaded_messages);
        Notification notification = new Notification();
        notification.icon = R.drawable.inc_msg_animated;
        Intent i = new Intent(this, main.class);
        i.setAction("OPEN_CHAT");
        PendingIntent contentIntent = PendingIntent.getActivity(this, 0, i, 0);
        notification.setLatestEventInfo(this, text, utilities.match(getString(R.string.s_x_messages_from_y_contacts), new String[]{String.valueOf(dump.messages), String.valueOf(dump.contacts)}), contentIntent);
        int flags = 0;
        //if(led){
