...
public class ChatActivity extends ExFragment implements Callback {
    ...
    public Dialog onCreateDialog(final int type) {
        ...
        switch (type) {
                ...
                if (contact.itIsTransport()) {
                    if (contact.profile.getTransportByID(contact.getTransportId()).params.auth_supported) {
                        if (contact.auth_flag)
                            adapter.put(resources.context_menu_icon, resources.ctx.getString(R.string.s_ask_suth), 0);
                        if (contact.haveAuthReq()) {
                            adapter.put(resources.context_menu_icon, resources.ctx.getString(R.string.s_accept_auth), 1);
                            adapter.put(resources.context_menu_icon, resources.ctx.getString(R.string.s_decline_auth), 2);
                        }
                    }
                    if (contact.getStatus() != -1) {
                        adapter.put(resources.context_menu_icon, resources.ctx.getString(R.string.s_show_status_message), 7);
                    ...
                    if (contact.getStatus() != -1) {
                        adapter.put(resources.context_menu_icon, resources.ctx.getString(R.string.s_show_status_message), 7);
                        adapter.put(resources.context_menu_icon, resources.ctx.getString(R.string.s_send_file), 6);
                    }
                }
            ...
            case 0x1://Auth reason input
                ...
                reason_input.setText(resources.ctx.getString(R.string.s_type_in_auth_req_template));
            ...
            case 0x3://Message context
                ...
                adapter.setPadding(10);
                adapter.put(resources.context_menu_icon, resources.ctx.getString(R.string.s_do_quote), 3);
            ...
            switch (id) {
                ...
                case 7:
                    if (contact.haveExtendedStatusDescription()) {
                        ...
                        dialog_for_display.header = resources.ctx.getString(R.string.s_information);
                        dialog_for_display.text = resources.ctx.getString(R.string.s_contact_didnt_have_status_message);
                        removeDialog(2);
                        showDialog(2);
                    }
                    break;
    ...
    private void initViews() {
        ...
        if (ColorScheme.initialized && !ColorScheme.internal)
            chat_top_panel.setBackgroundColor(ColorScheme.getColor(26));
        if (ColorScheme.initialized && !ColorScheme.internal)
            chat_bottom_panel.setBackgroundColor(ColorScheme.getColor(25));
        if (ColorScheme.initialized && !ColorScheme.internal)
            messages_list.setBackgroundColor(ColorScheme.getColor(27));
