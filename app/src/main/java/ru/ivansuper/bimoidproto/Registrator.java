...
public class Registrator {
    ...
    private void handleServerHelloReply(BEX bex) {
        ...
        if (tld == null) {
            RESULT_MSG = res.getString(R.string.s_incorrect_server_answer);
            socket.disconnect();
            return;
        }
        boolean registration_enabled = tld.getData().readBoolean();
        if (!registration_enabled) {
            RESULT_MSG = res.getString(R.string.s_registration_not_enabled);
            socket.disconnect();
            return;
        }
        doSendRegData();
    ...
    private void handleServerRegistrationReply(BEX bex) {
		 ...
		 */
            case 0x0:
                RESULT_MSG = res.getString(R.string.s_registration_success);
                disconnected_by_server = false;
                STATUS = SUCCESS;
                break;
            case 0x1:
                RESULT_MSG = res.getString(R.string.s_reg_err1);
                break;
            case 0x2:
                RESULT_MSG = res.getString(R.string.s_reg_err2);
                break;
            case 0x3:
                RESULT_MSG = res.getString(R.string.s_reg_err3);
                break;
            case 0x4:
                RESULT_MSG = res.getString(R.string.s_reg_err4);
                break;
            case 0x5:
                RESULT_MSG = res.getString(R.string.s_reg_err5);
                break;
            case 0x6:
                RESULT_MSG = res.getString(R.string.s_reg_err6);
                break;
        }
        socket.disconnect();
    }
